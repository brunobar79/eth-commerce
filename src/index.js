class EthCommerce {
	...
	render(options, errorCallback, successCallback, tryMetamascara=true) {
	...
	renderNoWeb3(){
		...
		div.classList.add('eth-qr-wrapper');

		const p = document.createElement('p');
		p.classList.add('eth-web3-missing');
		p.innerHTML('Looks like your browser doesn\'t support dApps. Install <a href="https://metamask.io" />metamask.io</a> Try visiting this website from a mobile app like Cypher, Toshi or Status...');

		div.appendChild(p);

		document.getElementById(targetElement).appendChild(p);
	}
	...
	waitForConfirmation(tx) {
		const MIN_CONFIRMATIONS = 5, INTERVAL = 5;
		let txBlockNumber = null, confirmations = 0;
	...
	sendTransaction(account, address, amount) {
		...
		return new Promise((resolve, reject) => {
			web3.eth.sendTransaction(tData, (error, txID) => {
