class EthCommerce {
	...
	injectMetamascara(options) {
		...
		metamascara.addEventListener('load',_=> {
			...
			ethJsScript.src = 'https://cdn.jsdelivr.net/npm/ethjs@0.3.0/dist/ethjs.min.js';
			const ref = document.querySelector('script');
	...
	renderNoWeb3(targetElement){
		...
		const str = 'Looks like your browser does not support dApps. Install <a  target="_blank" href="https://metamask.io">metamask.io</a> or try visiting this website from a mobile app like <a target="_blank" href="https://www.cipherbrowser.com/">Cypher</a>, <a target="_blank" href="http://www.toshi.org/">Toshi</a> or <a target="_blank" href="https://status.im/">Status</a>';
		p.innerHTML = str;
		div.appendChild(p);
				...
				if(!error){
					web3.eth.getGasPrice((error, gasPrice) => {
						if (error) {
							...
							web3.eth.sendTransaction({...tData, gas: gas, gasPrice: gasPrice * 2}, (error, txID) => {
								if (error) {
									reject(error);
								} else {
